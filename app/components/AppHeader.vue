...
function handleConnectClick() {
  ...
  console.log('[Header] Connect clicked, mode:', mode);
  if (mode === 'inject') {
...
onMounted(() => {
  ...
  function initConnector() {
    console.log('[Header] Initializing Connector2. Mode:', currentConnectorMode.value);
    if (window.Connector2) {
  ...
  if (window.Connector2) {
    ...
    const script = document.getElementById("connector-header-script");
    if (script) {
  ...
  modeWatcherInterval = setInterval(() => {
    ...
    if (currentConnectorMode.value !== rawMode) {
      console.log('[Header] Hot swapping mode to:', rawMode);
